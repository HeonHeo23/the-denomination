...
| Domain-aware UI projection          | Implemented | `src/ui/formatValue.ts`, `src/ui/graph`, `src/ui/game`, `src/ui/panels`                | A fixed-viewport, tactile strategy-table presentation keeps formatting and meters domain-aware. The graph opens on the first Scenario category at a readable scale, provides keyboard category/Overview navigation and a minimap, and preserves free graph interaction. Node types use distinct piece construction and analytical dialogue expose contribution direction, source activity, and Effect inertia. Completed turns derive disposable changed-node and changed-Effect feedback, run an exact 1000ms board-wide reveal, and then open the Year in Review without mutating canonical state. |
