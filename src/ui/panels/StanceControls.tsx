...
function StanceControl({ definition, value, onApply }: StanceControlProps) {
  ...
  const step = definition.control.kind === 'continuous' ? definition.control.step : undefined
  ...
  return (
    <article className="stance-control">
      ...
      {definition.control.kind === 'continuous' ? (
        ...
        <select value={draft} onChange={(event) => setDraft(Number(event.target.value))}>
      ...
      <div className="stance-control__footer">
        ...
        <button type="button" disabled={draft === value} onClick={() => onApply(draft)}>
          Apply
        </button>
      </div>
    </article>
...
export function StanceControls({ scenario, state, onApply }: StanceControlsProps) {
