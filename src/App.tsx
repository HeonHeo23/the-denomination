...
function App() {
  ...
  const resource = exampleScenario.nodes.find((node) => node.type === 'resource')
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        ...
        <button className="advance-button" type="button" onClick={session.nextTurn}>
      ...
      <main>
        ...
        <section className="simulation-workspace">
          ...
          <footer className="statusbar">
            ...
            <button type="button" onClick={session.reset}>Reset scenario</button>
          </footer>
        </section>
        ...
        <aside className="chronicle-panel">
          ...
          {session.state.history.length === 0 ? (
            ...
            <ol className="chronicle-list">
              {[...session.state.history].reverse().slice(0, 8).map((entry) => (
                <li key={entry.id}>
            ...
            <div className="active-grudges">
              ...
              {session.state.grudges.map((grudge) => (
                <div key={grudge.id}>
                  ...
                  <strong>{grudge.magnitude > 0 ? '+' : ''}{grudge.magnitude.toFixed(3)}</strong>
                </div>
              ))}
            </div>
          )}
      ...
      {pendingDilemma && (
        <DilemmaPanel
          ...
          onChoose={(choiceId) => session.resolveDilemma(pendingDilemma.id, choiceId)}
        />
      )}
    </div>
  )
