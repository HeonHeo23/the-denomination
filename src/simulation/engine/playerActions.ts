import type { ScenarioDefinition, StanceDefinition } from '../domain/definitions'
...
function changeStance(
  ...
  if (amountChanged === 0) return reject(state, 'That Stance is already selected.')
  if (stance.cost?.maxChange !== undefined && amountChanged > stance.cost.maxChange) {
    return reject(
      state,
  ...
  if (stance.cost) {
    ...
    if (resource.value < cost) {
      return reject(state, `Not enough Authority. This change costs ${cost.toFixed(1)}.`)
    }
...
function resolveDilemma(
  ...
  if (!dilemma || !choice) return reject(state, 'That Dilemma choice is invalid.')
