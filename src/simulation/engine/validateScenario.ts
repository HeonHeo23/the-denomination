...
export function validateScenario(scenario: ScenarioDefinition): readonly string[] {
  const errors: string[] = []
  ...
  for (const node of scenario.nodes) {
    ...
    if (
      ...
      (node.baselineValue < node.domain.min || node.baselineValue > node.domain.max)
    ) {
  ...
  for (const effect of scenario.effects) {
    if (effectIds.has(effect.id)) errors.push(`Duplicate effect id: ${effect.id}`)
  ...
  for (const incident of [...scenario.events, ...scenario.dilemmas]) {
    ...
    for (const influence of incident.influences) {
      if (influence.source !== '_random_' && !nodeIds.has(influence.source)) {
        errors.push(`${incident.id} references missing influence ${influence.source}`)
      }
    }
    ...
    for (const consequences of consequenceGroups) {
      for (const consequence of consequences) {
        if (!nodeIds.has(consequence.target)) {
          errors.push(`${incident.id} references missing target ${consequence.target}`)
        }
        if (
  ...
  for (const node of scenario.nodes) {
    ...
    const resource = scenario.nodes.find(({ id }) => id === node.cost?.resourceId)
